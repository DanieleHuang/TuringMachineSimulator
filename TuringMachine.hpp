...
class TuringMachine {
  private:
    ...
    vector<int> * currStates;   //the state we are on
  ...
  public:
    ...
    ~TuringMachine();
    //hashmap key: currState, value:
    // { Hashmap key: input, value: ReplaceChar++Move++nextState }
    map< int , map< char , vector<char*>  > > * transitions;
    bool loadFromFile( /*const char * in_filename */);

    //compute next state deterministically according to input character
    char* computeNextState(int *currIndex,int *currState, char* input, char* repMovNext);

    //run the string on the given turing machine
    bool computeDet( char* input);

    bool computeNonDet(char* input);
    int getMaxLoops();
    ...
    vector<char*> * getRMNVector(int sState, char input);
